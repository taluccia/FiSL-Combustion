---
title: "Point file for Landscape 1"
author: "Anna Talucci"
date: '2022-08-11'
...
Because fires in 2015 &2016 were less than 40 acres these do not registrar for thermal hotspots, and were excluded eventually because not FWI data.
...
```
```{r eval=FALSE, include=FALSE}
st_write(intersect, "../outputs/Landscape1BurnedAreaPoly.shp", driver="ESRI Shapefile")
```
...
# Combine fires
```{r}
fires_2004_2005 = dplyr::bind_rows(fp1L1_v1, fp5L1_v1)
fires_2004_2005
...
```{r}
burned_2004_2005 = st_join(l1pts, fires_2004_2005) %>% drop_na(.)
```
```{r}
...
```{r}
burned2 = burned %>% 
  mutate(UniqueId = random_id(n= 154637, bytes = 4)) %>%
  dplyr::select(UniqueId, lat:geometry)
```
...
```
```{r}
burned2_2004_2005 = burned_2004_2005 %>% 
  mutate(UniqueId = random_id(n= 154243, bytes = 4)) %>%
  dplyr::select(UniqueId, lat:geometry)
```

```{r}
head(burned2_2004_2005)
```

```{r}
n_distinct(burned2_2004_2005$UniqueId)
```
...
# Rename Columns
```{r}
burned3 = burned2_2004_2005 %>% 
  rename( ID1 = UniqueId, ID2 = FIREYEAR) %>%
  dplyr::select(ID1, ID2, NAME, FIREID, lat, lon, geometry)
```
...
```{r eval=FALSE, include=FALSE}
st_write(burned2, "../outputs/Landscape1/Landscape1_burned_points.shp", driver="ESRI Shapefile")
```

# Convert to WGS 84

```{r}
burned3_wgs = st_transform(burned3, crs=wgs_proj)
```

```{r eval=FALSE, include=FALSE}
st_write(burned3_wgs, "../outputs/Landscape1/Landscape1_burned_points_wgs.shp", driver="ESRI Shapefile")
```
